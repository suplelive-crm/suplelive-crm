/*
  # Add Products and Baselinker Sync Tables

  1. New Tables
    - `products` - Product inventory management
    - `baselinker_sync` - Baselinker integration sync tracking
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'products' AND policyname = 'Users can manage their workspace products'
  ) THEN
...
-- Create policies for baselinker_sync (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'baselinker_sync' AND policyname = 'Users can manage their workspace baselinker sync'
  ) THEN
